...
const example = {}
example.FLAC_HEADER = [0x66, 0x4C, 0x61, 0x43];
example.MP3_HEADER = [0x49, 0x44, 0x33];
example.OGG_HEADER = [0x4F, 0x67, 0x67, 0x53];
example.M4A_HEADER = [0x66, 0x74, 0x79, 0x70];
example.WMA_HEADER = [
    0x30, 0x26, 0xB2, 0x75, 0x8E, 0x66, 0xCF, 0x11,
    0xA6, 0xD9, 0x00, 0xAA, 0x00, 0x62, 0xCE, 0x6C,
]
example.WAV_HEADER = [0x52, 0x49, 0x46, 0x46]
example.AudioMimeType = {
    mp3: "audio/mpeg",
    flac: "audio/flac",
    m4a: "audio/mp4",
    ogg: "audio/ogg",
...
example.IXAREA_API_ENDPOINT = "https://stats.ixarea.com/apis"


// Also a new draft API: blob.arrayBuffer()
example.GetArrayBuffer = async function (blobObject) {
...
example.GetFileInfo = function (artist, title, filenameNoExt, separator = "-") {
 ...
 * @return {string}
 */
example.GetMetaCoverURL = function (metadata) {
    let pic_url = "";
...
example.IsBytesEqual = function (first, second) {
 ...
 * @return {string}
 */
example.DetectAudioExt = function (data, fallbackExt) {
...
example.GetWebImage = async function (pic_url) {
    try {
...
example.WriteMp3Meta = function (audioData, artistList, title, album, pictureData = null, pictureDesc = "Cover") {
